...
const BlogsAdmin = () => {
  ...
  const handleDeleteConfirmed = async () => {
    try {
      ...
      console.error("Gagal menghapus blog:", error.response?.data?.message);
        ...
        <main className="px-4 sm:px-2 md:px-2 py-10 w-full">
          ...
          <div className="flex flex-wrap gap-2 mb-4 w-full">
            <input
              ...
              className="md:w-full md:max-w-2xl border border-gray-300 rounded-md px-8 py-2 text-sm"
              value={searchInput}
              onChange={(e) => setSearchInput(e.target.value)}
            />
            <div
              className="w-10 h-9 ml-3 md:ml-10 bg-orange-500 rounded-md shadow-[0_4px_0_0_#b45309] flex items-center justify-center cursor-pointer"
              onClick={() => {
            ...
            <button
              ...
              className="flex items-center ml-1 md:ml-2 text-white bg-orange-500 hover:bg-orange-400 text-sm font-semibold rounded-md shadow-[0_4px_0_0_#b45309] px-6 py-2"
            >
              + Tambah Blog
            </button>
          </div>
            ...
            {blogs.length === 0 ? (
              ...
              blogs.map((blog) => (
                ...
                >
                  ...
                  <h2 className="text-md md:text-lg font-semibold mb-2 text-left">
                    {blog.title}
                  </h2>
                  <p
                    className="text-sm text-gray-700 mb-4 text-left prose-content"
